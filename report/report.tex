...
\centering
\textbf{Mauricio Gabriel Ram\'{i}rez Rubio} \\[0.2em]
Data Engineering --- 9th \\
...
\begin{minipage}[t]{0.30\textwidth}
\centering
\textbf{Cesar Arturo Balam Euan } \\[0.2em]
Data Engineering --- 9th \\
Universidad Polit\'{e}cnica de Yucat\'{a}n \\
Km.\ 4.5 Carretera M\'{e}rida --- Tetiz \\
Tablaje Catastral 4448. CP 97357 \\
...
\begin{minipage}[t]{0.30\textwidth}
\centering
\textbf{Rodrigo Garc\'{i}a Mart\'{i}nez} \\[0.2em]
Data Engineering --- 9th \\
Universidad Polit\'{e}cnica de Yucat\'{a}n \\
Km.\ 4.5 Carretera M\'{e}rida --- Tetiz \\
Tablaje Catastral 4448. CP 97357 \\
...
\begin{minipage}[t]{0.30\textwidth}
\centering
\textbf{Diego De Gante P\'{e}rez } \\[0.2em]
Data Engineering --- 9th \\
Universidad Polit\'{e}cnica de Yucat\'{a}n \\
Km.\ 4.5 Carretera M\'{e}rida --- Tetiz \\
Tablaje Catastral 4448. CP 97357 \\
...
\begin{minipage}[t]{0.30\textwidth}
\centering
\textbf{Roselyn Guadalupe Polanco Gonz\'{a}lez} \\[0.2em]
Data Engineering --- 9th \\
Universidad Polit\'{e}cnica de Yucat\'{a}n \\
Km.\ 4.5 Carretera M\'{e}rida --- Tetiz \\
Tablaje Catastral 4448. CP 97357 \\
...
% --- Entities ---
\node[entity] (categories) at (0,4) {
    \textbf{categories}\\[2pt]
    \rule{3.4cm}{0.4pt}\\[2pt]
    \underline{category\_id} PK\\
    name\\
    description\\
    created\_at
};
...
\node[entity] (order_items) at (5.5,0) {
    \textbf{order\_items}\\[2pt]
    \rule{3.4cm}{0.4pt}\\[2pt]
    \underline{item\_id} PK\\
    order\_id FK\\
    product\_id FK\\
    quantity\\
    unit\_price
...
\node[entity] (payments) at (11,-4) {
    \textbf{payments}\\[2pt]
    \rule{3.4cm}{0.4pt}\\[2pt]
    \underline{payment\_id} PK\\
    order\_id FK\\
    amount, method\\
    status\\
    transaction\_ref\\
...
% --- Relationships ---
\draw[rel] (categories.south) -- (products.north)
    node[midway, right] {1:N};
\draw[rel] (products.east) -- (order_items.west)
    node[midway, above] {1:N};
\draw[rel] (orders.west) -- (order_items.east)
    node[midway, above] {1:N};
\draw[rel] (customers.south) -- (orders.north)
    node[midway, right] {1:N};
\draw[rel] (orders.south) -- (payments.north)
    node[midway, right] {1:1};

\end{tikzpicture}
\caption{Entity-Relationship diagram of the e-commerce schema (3NF). Arrows point from the ``one'' side to the ``many'' side of each relationship.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img2_generator_logs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img3_cdc_worker_logs.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img4_update_verification.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img5_row_count_comparison.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img6_postgres_explain.png}
    \caption{PostgreSQL query plan example (Q4 --- product performance).}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img7_clickhouse_explain.png}
...
available at: \url{https://github.com/ddgant/cdc-postgres-clickhouse}
